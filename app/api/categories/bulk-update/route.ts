...
export async function POST(request: Request) {
  ...
  if (!ids.length) return NextResponse.json({ success: false, message: "No categories selected." }, { status: 400 });
  ...
  const result = await db.category.updateMany({ where: { id: { in: ids } }, data });
  return NextResponse.json({ success: true, message: `${result.count} categories updated.`, data: { updated: result.count } });
}
