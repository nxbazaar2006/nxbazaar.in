...
const existingProduct = await db.product.findUnique({ where: { slug: productData.slug, }, }); if (existingProduct) { return NextResponse.json( { data: null, message: `Product ( ${productData.title}) already exists in the Database`, }, { status: 409 } ); }
    ...
    const newProduct = await db.$transaction(async (prisma) => {
      ...
      }); const actor = await historyActorFromSession(session); const productHistoryBase = historyBase({ ...product, actor }); await createProductHistory(prisma, { ...productHistoryBase, action: ProductHistoryAction.PRODUCT_CREATED, oldValue: null, newValue: { title: product.title, productCode: product.productCode, categoryId: product.categoryId, subCategoryId: product.subCategoryId, sellerId: product.userId, brand: product.brandId, tags: product.tags, status: product.status }, }); await syncProductAttributesAndVariants({ prisma, productId: product.id, productType: product.productType, attributes, variants: payload.variants || [], actor, }); return prisma.product.findUnique({ where: { id: product.id }, include: productInclude }); }); if (newProduct) { await safelyEnqueueTranslationJobs({ entityType: "PRODUCT", entityId: newProduct.id, sourceHash, requestedByUserId: session.user.id, }); } revalidatePath("/dashboard/products"); revalidatePath("/dashboard/products/new"); return NextResponse.json(newProduct); } catch (error) { console.error(error); return NextResponse.json( { message: prismaUniqueMessage(error, "Failed to create Product"), error, }, { status: 500 } ); }
}
