...
This file contains older findings from the original audit. The following auth findings were rechecked and updated:

- Auth.js migration is complete at the code level using root `auth.ts`.
- Live `getServerSession(authOptions)` and `withAuth` usage was removed.
- `proxy.ts` now exports the Auth.js v5 proxy function and protects `/admin`, `/seller`, `/dashboard`, `/account`, and `/checkout`.
- Credentials policy now rejects unknown email/wrong password generically, inactive accounts, and unverified `ADMIN`, `MODERATOR`, `SELLER`, and `FARMER` accounts.
- Active `USER` verification policy is preserved.
- Session/JWT propagation keeps `id`, `email`, `role`, `status`, and `emailVerified`.
- Prisma adapter support added with `Account`, `Session`, and `VerificationToken`.
- UploadThing reads JWT with `AUTH_SECRET`, not `NEXTAUTH_SECRET`.

Verification: direct `tsc`, `eslint`, `prisma validate`, `prisma generate`, `next build`, and `npm test -- --test-reporter=spec` passed. pnpm commands still fail because `C:\pnpm-store\v11` is not writable in this environment.

NextAuth uses credentials, PrismaAdapter and JWT sessions. Session callbacks expose user id, role, status and emailVerified. The proxy only gates /dashboard by authentication. Static scan found many API mutation routes without session checks.
...
- Evidence: Lines 3-29 export POST, read name/password/email from request JSON, construct newStaff with password, and return it; no getServerSession or role guard is present.
...
- Classification: Security risks
- Evidence: Lines 86-93 export GET and return db.user.findMany() without session or role checks.
...
- Classification: Security risks
- Evidence: Lines 4-29 read only id from request JSON and set emailVerified true; verificationToken is not checked.
...
- Classification: Security risks
- Evidence: Lines 4-31 read id and password from JSON and update the user password without checking reset token or session ownership.
...
- Classification: Security risks
- Evidence: Lines 4-15 return orders for params.id without getServerSession or owner/admin check.
...
- Classification: Security risks
- Evidence: Lines 6-21 read checkoutFormData.userId from request JSON; lines 39-52 write that userId into the created order; no session is checked.
...
- Classification: Security risks
- Evidence: Lines 6-35 accept POST category creation and perform db.category.findUnique/create flow with no session or role guard. Static scan found many unauthenticated mutation routes for banners, coupons, categories, markets, HSN, subcategories, trainings, orders and users.
...
- Classification: Security risks
- Current status: Fixed in Task 1.
- Evidence: `proxy.ts` exports the Auth.js v5 proxy from `auth.ts`; `auth.ts` protects `/admin`, `/seller`, `/dashboard`, `/account`, and `/checkout` with route-role checks.
- File path: auth.ts
- Description: Dashboard and protected route authorization now uses the Auth.js v5 proxy and role matrix.
- Business impact: Direct URL access is constrained by route role checks before protected pages are served.
- Verification steps: Sign in as USER and request admin/seller dashboard URLs; expect redirect/denial. Direct build/type/test verification passed.
- Estimated complexity: Medium

### NX-HIGH-003 - High - Seller data isolation

- Classification: Security risks
- Evidence: Lines 15-25 fetch all sales through getData("sales") and then filter by vendorId in the page component; app/api/sales/route.tsx returns sales without session checks.
...
- Classification: Security risks
- Evidence: Lines 8-49 define upload routes and return uploadedBy "JB" in onUploadComplete; no middleware/auth metadata is configured.
...
### NX-MED-011 - Medium - Error disclosure

- Classification: Security risks
- Evidence: Lines 207-212 return JSON containing raw error; similar error, fields appear across many API routes.
- File path: app/api/orders/route.tsx:207
- Description: API routes return raw error objects to clients.
- Business impact: Implementation details or sensitive database errors can leak to users/attackers.
- Recommended solution: Log server-side, return safe error codes/messages, and map expected validation errors explicitly.
- Verification steps: Force Prisma error and response contains only safe code/message, not stack/database object.
- Estimated complexity: Medium

### NX-MED-012 - Medium - Auth status enforcement

- Classification: Security risks
- Current status: Fixed in Task 1.
- Evidence: `lib/auth-credentials.ts` calls `canCredentialsUserSignIn`; `lib/auth-policy.ts` rejects inactive accounts and unverified `ADMIN`, `MODERATOR`, `SELLER`, and `FARMER` accounts.
- File path: lib/auth-credentials.ts
- Description: Inactive and unverified privileged accounts are rejected during credentials authorization.
- Business impact: Suspended sellers/admins/moderators/farmers cannot receive an authenticated session through credentials login.
- Verification steps: Added tests for inactive account, unverified seller, verified seller, active user policy, and role/session propagation.
- Estimated complexity: Small

## Technical Debt

### NX-LOW-001 - Low - Environment validation

- Classification: Technical debt
- Evidence: getAppEnv validates env with Zod at lines 39-47, but db.ts reads process.env.DATABASE_URL directly at lines 8-10 and several routes/components read process.env directly.
- File path: lib/env.ts:39
- Description: Central environment validation exists but is not consistently used.
- Business impact: Missing config fails late at runtime, as seen in Redis check failure.
- Recommended solution: Import getAppEnv in server modules and expose a typed public env helper for NEXT_PUBLIC values.
- Verification steps: Missing required env fails at startup with one clear message before serving requests.
- Estimated complexity: Small
