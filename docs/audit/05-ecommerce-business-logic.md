...
Product, category, subcategory, HSN, cart, checkout, order item, sale, seller order, payment and inventory models or UI paths exist. The highest-risk gaps are checkout identity, atomic inventory, seller-wise stock, order totals, and incomplete payment/refund flows.
...
### NX-MED-003 - Medium - Product validation

- Classification: Confirmed defects
- Evidence: Lines 3-8 validate only categoryId, subCategoryId, hsnCodeId and hsnOverrideEnabled; NewProductForm useForm at lines 234-245 has no Zod resolver.
- File path: lib/validations/product.ts:3
- Description: Product form/API validation is incomplete for prices, stock, SKU, barcode, variants and SEO fields.
- Business impact: Invalid catalog data can reach APIs and fail late or persist inconsistently.
- Recommended solution: Create a full shared Zod product schema and use it in React Hook Form and API handlers.
- Verification steps: Negative price, missing SKU, duplicate barcode and invalid variants should produce field/API errors.
- Estimated complexity: Medium
...
### NX-MED-006 - Medium - Invoice accuracy

- Classification: Confirmed defects
- Evidence: Invoice component renders an order but order creation does not persist order aggregate GST totals, and invoice page app/(back-office)/dashboard/orders/[id]/invoice/page.tsx lines 5-7 fetches any order id without local auth check.
- File path: components/Order/SalesInvoice.tsx:1
- Description: Invoice rendering depends on incomplete and weakly protected order data.
- Business impact: GST invoices can be inaccurate or exposed by URL.
- Recommended solution: Persist immutable invoice snapshots at order/payment time and protect invoice route by owner/admin/seller scope.
- Verification steps: Invoice totals match OrderItem tax snapshots and unauthorized users cannot load other invoices.
- Estimated complexity: Medium
...
- NX-HIGH-004: Fixed by moving checkout stock deduction to conditional `updateMany` statements inside the existing transaction. Variant stock requires `stock >= quantity`; simple product stock requires `productStock >= quantity`.
- NX-HIGH-006: Fixed by adding `lib/orders/checkout-calculations.ts`, persisting order-level GST totals in checkout, creating seller-wise `SellerOrder` settlement totals, storing order item regular/sale price snapshots, and linking each order item to its seller order.

Partially fixed finding:

- NX-HIGH-005: Partially fixed by updating seller-wise `Inventory.quantity` during checkout. Full database enforcement still needs a migration because the existing nullable composite unique constraint does not fully protect simple-product inventory rows in PostgreSQL.

Migration note:

- No migration was created in this pass. The remaining schema change would add a partial unique index for simple-product `Inventory` rows. It should be preceded by a duplicate-data report and rollback would drop only that index.

Verification commands:

- `npm test -- --test-reporter=spec`: passed, including GST and checkout total snapshot tests
