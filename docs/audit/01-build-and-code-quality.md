# Build, Scripts and Code Quality

## Task 1 Update - 2026-07-25

Verification was rerun after Auth.js v5 and AI MVP fixes.

pnpm status:

- `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`, and `pnpm exec prisma validate` all fail before running project code with `unable to open database file`.
- `pnpm store path` resolves to `C:\pnpm-store\v11`.
- A write test to `C:\pnpm-store` failed with `EPERM`.
- This is a pnpm store permission/database issue, not a TypeScript/Next.js project failure.
...
package.json defines dev, build, start, lint, typecheck, Redis, translation worker and migration helper scripts. No generic test script exists. Dependency health was assessed from local manifests only; no network vulnerability database was queried in this restricted environment.
...
- Classification: Confirmed defects
- Evidence: Lines 16-18 set typescript.ignoreBuildErrors = true.
...
### NX-MED-001 - Medium - TypeScript strictness

- Classification: Technical debt
- Evidence: Lines 10-12 set allowJs true, skipLibCheck true and strict false.
- File path: tsconfig.json:10
- Description: TypeScript is not running in strict mode.
- Business impact: Null, unsafe field access and weak domain contracts can escape compile-time detection.
- Recommended solution: Enable strict incrementally by folders, starting with lib/domain, API routes and checkout.
- Verification steps: tsc --noEmit passes with strict true or a staged strict tsconfig for critical folders.
- Estimated complexity: Large

### NX-MED-002 - Medium - Lint coverage

- Classification: Technical debt
- Evidence: Lines 29-34 disable react-hooks/rules-of-hooks, react-hooks/set-state-in-effect, react/jsx-key and other rules.
- File path: eslint.config.mjs:29
- Description: Important React correctness rules are disabled.
- Business impact: Hook misuse and missing keys can ship despite lint passing.
- Recommended solution: Re-enable rules and fix resulting violations in small batches.
- Verification steps: eslint fails on intentional hook rule violations and passes after fixes.
...
### NX-LOW-003 - Low - Dependency health

- Classification: Technical debt
- Evidence: Dependencies include both flowbite/flowbite-react and shadcn/Radix-style components; carousel libraries include nuka-carousel, react-multi-carousel and swiper.
- File path: package.json:18
- Description: UI/dependency stack has overlapping packages.
- Business impact: Bundle size, design consistency and maintenance burden increase.
- Recommended solution: Choose primary UI and carousel stacks; remove unused packages after usage audit.
- Verification steps: Bundle analyzer confirms removed unused dependencies and no import regressions.
- Estimated complexity: Medium
